...
class Simulation:
    ...
    # It also protects internal state: without a matching @x.setter, any assignment raises AttributeError.
    # Use @x.setter only when the attribute needs to be writable from outside the class.
    @property
    ...
    def render(self, mode="rgb_array"):
        if mode != "rgb_array":
            return np.array([])
        base_pos = self._robot.get_base_position()
    ...
    def _generate_terrain(self, terrain_id, terrain_type, _pybullet_client):
